...
Bob.prototype.build = function (taskNames, cb) {
  ...
  this._init(taskNames, function (err, settings) {
    if (err) { cb(err); } else {
      self._commands(taskNames, settings, function (err, commands) {
        if (err) { cb(err); } else {
...
Bob.prototype._init = function (taskNames, cb) {
  ...
  function installDeps(err, settings) {
    if (err) { cb(err); } else {
      ...
      deps.install(taskTypeNames, { dir: self.opts.bobDir }, function (err) {
        cb(err, settings);
      });
    }
  }
 ...
 * @param {Function} cb: standard cb(err, result) callback
 */
Bob.prototype._commands = function (taskNames, settings, cb) {
  const TPL_PARAMS = {
