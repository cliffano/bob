 ...
 * @param {Object} tasks: Bob's tasks configuration
 ...
 */
function Bob(dir, tasks, custom, pkg) {
  this.dir = dir || '.';
  this.tasks = tasks || {};
  this.custom = custom || {};
  this.pkg = pkg || {};
...
Bob.prototype.external = function (targets, mode, verbose, cb) {
  ...
  function _checkOverrides(target, targets, i) {    
    _.keys(self.tasks[target].overrides.pkg).forEach(function (override) {
      if (bag.obj.exist(self.tasks[target].overrides.pkg[override], self.pkg)) {
        targets[i] = override;
        isOverridden = true;
      }
    });
  ...
  function _constructParams(target, targets, i) {
    ...
    var type = bag.obj.exist(target + '.type', self.custom) ?
        self.custom[target].type : self.tasks[target]['default'],
      tool = self.tasks[target].types[type];
