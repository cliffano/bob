...
function Bob(opts) {
  ...
  var bobJson = JSON.parse(fs.readFileSync(p.join(opts.bobDir, 'conf/bob.json'))),
  ...
  function _targets(targets) {
    ...
    // keys: Makefile targets
    var keys = bobJson.targets;

    // if target does not have a type, then assign the default type
    _.keys(keys).forEach(function (key) {

      var pos = targets.indexOf(key),
        type = pkg.bob[key].type;
      
      if (pos >= 0) {
        if (type === undefined) {
          type = keys[key].default;
        }
        targets[pos] = key + '-' + type;
      }
    });
