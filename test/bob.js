...
describe("testbob - init", function () {
  ...
  it("should load task, load config, and install dependencies", function (done) {
    ...
    this.mockTask
      .expects("load")
      .once()
      .withArgs(["task1", "task2"], "/somebobdir/conf/tasks")
      .callsArgWith(2, null, mockBobTasks);
    this.mockConfig
      .expects("load")
      .once()
      .withArgs(["task1", "task2"], "/someappdir")
      .callsArgWith(2, null, mockAppConfig);
...
describe("testbob - commands", function () {
  it("should construct commands and pass them to callback", function () {
        ...
        lint: {
          ...
          types: {
            jshint: {
              ...
              opts: { human: "", robot: "--jslint-reporter" },
              args: "lib/ test/",
            },
          },
        },
    ...
    bob._commands(taskNames, settings, function (err, results) {
      ...
      referee.assert.equals(
        results[0].exec,
        "/node_modules/jshint/bin/jshint --jslint-reporter lib/ test/",
      );
  ...
  it("should construct commands with pre and post opts and pass them to callback", function () {
      ...
      bobTasks: {
        lint: {
          ...
          types: {
            jshint: {
              bin: "{bob}/node_modules/jshint/bin/jshint",
              postOpts: { human: "", robot: "--jslint-reporter" },
              args: "lib/ test/",
            },
          },
        },
    ...
    bob._commands(taskNames, settings, function (err, results) {
      ...
      referee.assert.equals(
        results[0].exec,
        "/node_modules/jshint/bin/jshint lib/ test/ --jslint-reporter",
      );
      referee.assert.equals(results[0].meta.task, "lint");
      referee.assert.equals(results[0].meta.type, "jshint");
    });
