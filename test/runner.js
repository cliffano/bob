...
describe("testrunner - exec", function () {
  ...
  it("should write data to stream", function () {
    ...
    this.mockConsole
      .expects("log")
      .withExactArgs("%s | %s", "lint".cyan, "somecommand");
    this.mockChild
      .expects("exec")
      .withExactArgs("somecommand", opts, cb)
      .returns(mockChildProcess);
    this.mockFs
      .expects("createWriteStream")
      .withExactArgs("somedir/jshint.txt")
      .returns(mockStream);
...
describe("testrunner - execSeries", function () {
  ...
  it("should execute commands when there is no error", function (done) {
    ...
    this.mockConsole
      .expects("log")
      .withExactArgs("%s | %s", "test".cyan, "somecommand");
    this.mockChild
      .expects("exec")
      .withArgs("somecommand", sinon.match.object, sinon.match.func)
      .returns(mockChildProcess)
      .callsArgWith(2);
    this.mockFs
      .expects("createWriteStream")
      .withExactArgs("somedir/.bob/test/buster.txt")
      .returns(mockStream);
    const commands = [
  ...
  it("should pass error when an error occurs while executing command", function (done) {
    ...
    this.mockChild
      .expects("exec")
      .withArgs("somecommand", opts, sinon.match.func)
  ...
  it("should use tmp dir when task is clean", function (done) {
    ...
    this.mockConsole
      .expects("log")
      .withExactArgs("%s | %s", "clean".cyan, "somecommand");
    this.mockChild
      .expects("exec")
      .withArgs("somecommand", sinon.match.object, sinon.match.func)
      .returns(mockChildProcess)
    ...
    this.mockFs
      .expects("createWriteStream")
      .withExactArgs("sometmpdir/.bob/clean/rimraf.txt")
      .returns(mockStream);
    const commands = [
