...
describe("testcli - exec", function () {
  ...
  it("should contain commands with actions", function () {
    this.mockConsole
      .expects("log")
      .withExactArgs("%s | exit code 0", "SUCCESS".green);
    ...
    this.mockFs
      ...
      .once()
      .withExactArgs(p.join(process.cwd(), "./conf/commands.json"))
      .returns(JSON.stringify(this.commandsJson));
    this.mockFs
      .expects("readFileSync")
      .withExactArgs(p.join(process.cwd(), "./.bob.json"))
      .returns(JSON.stringify(this.bobJson));
    this.mockFs
      .expects("readFileSync")
      .withExactArgs(p.join(process.cwd(), "./package.json"))
      .returns(JSON.stringify(this.packageJson));
  ...
  it("should log error message when an error occured and there is no error code", function () {
    this.mockConsole
      .expects("error")
      .withExactArgs("%s | %s", "ERROR".red, "some error");
  ...
  it("should log failure message when an error occured with exit code", function () {
    this.mockConsole
      .expects("error")
      .withExactArgs("%s | exit code %d", "FAILURE".red, 123);
